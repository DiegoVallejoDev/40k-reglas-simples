...
function stratagemToHTML(stratagem) {
  ...
  return `
  <article class="card" id="${name.toLowerCase().replace(/ /g, '-')}" role="region" aria-label="${name}">
    ...
    `<div class="when-symbol" style="background-color:${color}; box-shadow: 1px 1px 3px ${color};">
              <img src="./svg/${icon}.svg" alt="${icon}" loading="lazy" style="width: 100%; height: 100%; transform: rotate(-90deg);">
            </div>`
  ).join('')
    }
...
document.addEventListener("DOMContentLoaded", function () {
  // Add stratagems to body using DocumentFragment for better performance
